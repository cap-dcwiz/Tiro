        ...
        for entity in entities:
            if self.config.by == "path":
                ...
                if r.status_code == httpx.codes.OK:
                    ...
                    result.append(
                        self.update_meta(data, category=entity.split(".")[-2].lower())
                    )
                else:
                    logging.error(
                        f"{self.__class__.__name__}[{self.name}] request error {r.status_code}: /points/{entity}"
                    )
                ...
                if r.status_code == httpx.codes.OK:
                    ...
                    logging.error(
                        f"{self.__class__.__name__}[{self.name}] request error {r.status_code}: /values/{entity}"
                    )
            else:
