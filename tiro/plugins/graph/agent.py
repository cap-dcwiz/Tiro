...
from tiro.core.utils import (
    PATH_SEP,
    insert_data_point_to_dict,
    split_path,
    format_regex,
)
...
class ArangoAgent:
    ...
    def create_graph(self, clear_existing: bool = True, clear_database: bool = False):
        ...
        for e_type, e_info in edges_info.items():
            if self.graph.has_edge_definition(e_type):
                self.graph.replace_edge_definition(
                    e_type, list(e_info["from_set"]), list(e_info["to_set"])
                )
            else:
    ...
    def insert_vertices_and_edges(
        self, vertices=None, edges=None, replace=False, insert_default_dp=False
    ):
        ...
        for v_type, v_value in vertices.items():
            ...
            if not v_collection.has(v_value["_key"]):
                ...
                if insert_default_dp and "value" not in v_value:
                    self.create_default_data_points(
                        v_type, v_value["_key"], v_value["path"]
                    )
            elif replace:
    ...
    def update(self, item: dict):
        ...
        self.insert_vertices_and_edges(
            g_info["vertices"], g_info["edges"], replace=True, insert_default_dp=True
        )

    def query_attributes_and_missing(
        self,
    ...
    def _query2(self, query_statement: str, **bind_vars) -> dict:
        db = self.db(create=False)
        start_vertex = f"{self.entity.name}/000"
        cursor = db.aql.execute(
            query_statement,
            bind_vars=bind_vars
            | dict(start_vertex=start_vertex, graph_name=self.graph_name),
        )
        data = {}
    ...
    def _format_df(self, data, tags: str | list[str]):
        if tags == "all":
            return pd.DataFrame(self.scenario.decompose_data("", data)).set_index(
                "asset_path"
            )
    ...
    def query_by_qpath(
        ...
        query_statement = QUERY_BY_QPATH_AQL.format(
            prune_statement=query_path.prune_statement(),
